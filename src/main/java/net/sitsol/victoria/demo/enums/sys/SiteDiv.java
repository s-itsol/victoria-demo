package net.sitsol.victoria.demo.enums.sys;
...
/**
 * victoriaデモ用-Enum定数「サイト種別」
 * 
 * @author vitoria-type-src-generater
 */
public enum SiteDiv implements VctEnumAccessible {

	/** パソコン ※1：パーソナルコンピュータ */
	PersonalComputer("1", "パーソナルコンピュータ")
	,
	/** スマホ ※2：スマートフォン */
	SmartPhone("2", "スマートフォン")
	,
	/** 携帯 ※3：携帯電話 */
	MobilePhone("3", "携帯電話")
	;
	 ...
	 *    アプリでは常にこちらの使用を推奨する。
	 */
	public static List<SiteDiv> getAllTypeList() {
		return new ArrayList<>( Arrays.asList(SiteDiv.values()) );
	}
	 ...
	 */
	public static SiteDiv getType(String code) {
		
		SiteDiv retEnumType = null;
		
		// 型ループ
		for ( SiteDiv enumType : SiteDiv.values() ) {
			// コード値が一致したら、戻り値として確定
...
//		; 
	...
	public static String getDecode(String code) {
		SiteDiv targetType = getType(code);
	 ...
	 */
	private SiteDiv(String code, String decode) {
		this.code = code;
		this.decode = decode;
	}
